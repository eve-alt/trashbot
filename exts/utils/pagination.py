...
class EmbedPaginator(Dialog):
    ...
    def __init__(self, client: discord.Client, pages: [discord.Embed], message: discord.Message = None):
        ...
        self.emoji_ids = (747116225833992193, 747116236260769833, 747116235224776765, 747116228748771338, 747116222990254090)
        self.control_emojis = []

        for x in self.emoji_ids:
        	emoj = self._client.get_emoji(x)
        	self.control_emojis.append(emoj)

        #('<:fprev:747116225833992193>', '<:prev:747116236260769833>', '<:next:747116235224776765>', '<:fnext:747116228748771338>', '<:stop:747116222990254090>')
        #self.control_emojis = ('⏮', '◀', '▶', '⏭', '⏹')
